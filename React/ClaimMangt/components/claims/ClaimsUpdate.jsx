...
const validClaimNumber = new RegExp(
...
class ClaimsUpdate extends React.Component {
  constructor(props) {
  ...
  componentDidMount() {
    ...
    axios
      .get(`http://localhost:7000/claims/${this.props.params.claimId}`)
      .then((res) => {
  ...
  handleChange(event) {
    ...
    switch (name) {
      case "claim_number":
        errors.claimNumber = validClaimNumber.test(value)
          ? ""
          : "Claim Number must be alphanumeric values !";
        break;
  ...
  submitClaim(e) {
    ...
    axios
      .put(
        "http://localhost:7000/claims/" + this.refs["emp_id"].value,
        claimObj
      )
      .then((res) => {
